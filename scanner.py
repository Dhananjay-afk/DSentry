...
def scan_text(text, filename="<string>"):
    """
    Scan a raw string for secrets.
    Used for library mode — no file needed.
    
    Usage:
        from scanner import scan_text
        results = scan_text("api_key = 'sk_live_abc123'")
    """
    findings = []
    ...
    return {
        ...
        "results": {filename: findings} if findings else {}
    }
...
def scan_github_url(url):
    ...
    try:
        ...
        print(f"  Clone complete. Scanning...")
        results = scan_directory(temp_dir)
        return results
    except subprocess.TimeoutExpired:
        return {"error": "Clone timed out. Repository may be too large."}
